...
class _RecipeCardState extends State<RecipeCard> {
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          _infoChip(Icons.person_outline_rounded, 'Serves ${(widget.servings * recipeYieldModifier).round()}'),
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      _scaleButton('Default', recipeYieldModifier == 1, () => setState(() => recipeYieldModifier = 1)),
                      const SizedBox(width: 8),
                      _scaleButton('Double ×2', recipeYieldModifier == 2, () => setState(() => recipeYieldModifier = 2)),
                      const SizedBox(width: 8),
                      _scaleButton('Triple ×3', recipeYieldModifier == 3, () => setState(() => recipeYieldModifier = 3)),
                    ],
                  ),
                    ...
                    return Padding(
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              _scaleIngredient(widget.ingredients[i].toString(), recipeYieldModifier),
                              style: const TextStyle(
                                fontFamily: 'Oswald',
                                fontSize: 14,
  ...
  String _scaleIngredient(String raw, double modifier) {
    if (modifier == 1) return raw;
    final match = RegExp(r'^\s*(\d+\s+\d+/\d+|\d+/\d+|\d*\.?\d+)').firstMatch(raw);
    if (match == null) return raw;
    final value = _parseQty(match.group(1)!);
    if (value == null) return raw;
    final rest = raw.substring(match.end);
    final separator = rest.startsWith(' ') || rest.isEmpty ? '' : ' ';
    return '${_formatQty(value * modifier)}$separator$rest';
  }
  ...
  String _formatQty(double v) {
    ...
    for (final f in fractions) {
      if ((frac - (f[0] as double)).abs() < 0.05) {
        return whole == 0 ? f[1] as String : '$whole ${f[1]}';
      }
    }
    return v.toStringAsFixed(2).replaceAll(RegExp(r'\.?0+$'), '');
  }

  Widget _scaleButton(String label, bool active, VoidCallback onTap) {
    return Expanded(
